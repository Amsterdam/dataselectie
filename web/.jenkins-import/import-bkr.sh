...
dc() {
	docker-compose -p ds_bkr -f ${DIR}/docker-compose.yml $*
}
...
dc run --rm importer python manage.py elastic_indices --recreate bkr

# create dataselectie HR indexes

dc run --rm importer /app/docker-index-bkr.sh

dc run --rm elasticsearch chmod -R 777 /tmp/backups

dc run --rm importer /app/elk-bkr-backup.sh
